...
class ChatRepository {
  Future<Either<FailureModel, Map>> getChatRooms() async {
    return await HttpHelper.handleRequest((token) async {
      return await HttpHelper.getData(
          linkUrl: ApiEndPoints.chatRooms, token: token);
    });
  }

  Future<Either<FailureModel, Map>> getChatRoomMessagesById(
      {required String id, int? page, int? limit}) async {
    return await HttpHelper.handleRequest((token) async {
      return await HttpHelper.getData(
          linkUrl:
              "${ApiEndPoints.chatRoomMessages}$id?number=$limit&page=$page",
          token: token);
    });
  }
  ...
  //   });
  // }
  Future<Either<FailureModel, Map>> sendMessage({
    required int id,
    required String content,
    required List<File> files,
  }) async {
    return await HttpFileHelper.handleRequest((token) async {
      return await HttpFileHelper.postMessage(
          url: '${ApiEndPoints.chatRoomMessage}$id',
          token: token,
          content: content,
          files: files);
    });
  ...
  Future<Either<FailureModel, Map>> createChatRoom(
